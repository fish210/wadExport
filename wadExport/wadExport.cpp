...
int main()
{
	...
	if (hdll == 0)
	{
		cout << "load error, not is libzstd.dll ; 模块加载失败 libzstd.dll\n" << endl;
		return 0;
	}
	...
	{
		FreeLibrary(hdll);
		cout << "load error not is ZSTD_decompress ; 找不到函数 ZSTD_getDecompressedSize\n" << endl;
		return 0;
	}
	...
	{		
		FreeLibrary(hdll);
		cout << "load error not is ZSTD_decompress ; 找不到函数 ZSTD_decompress\n" << endl;
		return 0;
	}
	//*********************************************************** Start
	cout << "Are you ready\n" << endl;
